...
namespace rv
{
	...
	public:
		...
		constexpr BasicIdentifier(const char* name) : m_hash(hash_t<H>(name)), m_name(name) {}
		...
		constexpr BasicIdentifier& operator= (const char* name) { m_hash = hash_t<H>(name); m_name = name; return *this; }
